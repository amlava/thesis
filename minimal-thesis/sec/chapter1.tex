...
\]
where $f\colon\R^n\to\R$ is a convex function and
$\mathcal{X}\subset\R^n$ is a convex set.
...
In this thesis, we consider composite stochastic optimization problems of the form
\begin{equation}
    ...
    \end{aligned}
\end{equation}
where $\xi$ is a random variable, and for any realization $z$ of $\xi$, $A(z)\in\R^{m\times n}$
and $b(z)\in\R^m$. We assume that $F_\xi$ is $L$-smooth, $\E(F_\xi)\colon\R^n\to\R$ is $\mu$-strongly convex, and
that $r\colon\R^n\to\R$ is Lipschitz continuous convex (but possibly nonsmooth). Examples of these problems
...
as is the case in modern machine learning, where the random variable $\xi$ models
...
\textit{stochastic subgradient} $g_k$ of $f$. Then we set $x_{k+1} = \proj(x_k - \eta_k g_k)$,
...
of \eqref{eq:main-problem} with rate $\mathcal{O}(1/k)$ for strongly convex objectives.
...
Where before we required $\pi(x) = 0$ for all feasible points,
...
Compute a subgradient of $f$ at $x_k$, denoted by $\tilde{\nabla}f(x_k)$;
sample a random index $i\in\{\,1,\dots,m\,\}$ and calculate the gradient $\nabla h_k^{\text{hub}}(x_k; a_i, b_i)$;
finally, update $x_{k+1} := x_k - \eta_k(\tilde{\nabla}f(x_k) + \gamma_k \nabla h_k^{\text{hub}}(x_k; a_i, b_i))$.
After a set amount of $K\in\N$ iterations, one computes a certain weighted average of
the iterates, which yields the final output $\bar{x}_K$.
...
sequece $(\bar{x}_k)_{k\in\N}$ satisfies
$\dist(\bar{x}_k, \mathcal{X}) = \mathcal{O}(\log^{\epsilon} k/ k)$
and $|f(\bar{x}_k) - f(x^\star)| = \mathcal{O}(\log^{\epsilon} k/ k)$,
where $x^\star$ denotes the solution to \eqref{eq:nedic-problem}.
\begin{algorithm}[t]
\caption{Incremental Gradient Method (Nedić et al. \cite{nedich2023huber})}
\label{alg:nedic-sgd}
...
such that
\[
    \pi_k(x) = \E(h_k( A(\xi)x - b(\xi) )) \enspace\text{and}\enspace \pi_k(x) \downarrow_{k\to\infty} \pi_{\ell_1}(x)
\]
for all $x\in\R^n$.
This more general treatment allows for more flexibility in the design of the method,
and includes the recent \textit{softplus penalty} introduced in \cite{li2025new}.

As opposed to the setting of \cref{alg:nedic-sgd},
we may not be able to calculate the gradient of our objective, $x\mapsto \E(F_\xi(x)) + r(x)$,
because $r$ may be nonsmooth
or because calculating the gradient of the
expectation functional, $\E(F_\xi(x))$, may be infeasible. Say, for example, because the
...
multiple iterations or even just once.
...
with nonsmooth objectives: For $\eta\in(0, \infty)$ and $r\colon\R^n\to\R$,
the proximal operator $\prox_{\eta r}\colon\R^n\to\R^n$ is defined as
\[
    \prox_{\eta r}(x) := \argmin_{u\in\R^n} \left\{\, r(u) + \frac{1}{2\eta} \norm{u - x}^2_2 \,\right\}.
\]
The proximal map allows us to only work with gradients of the differentiable terms in our objective.
...
$z$ from the distribution of $\xi$ as
$\nabla F_z(x) + \gamma_k A^\top(z)\nabla h_k(A(z)x - b(z))$.
...
a variance-reduced estimate of $\nabla \E(F_\xi(x)) + \gamma_k \nabla \pi_k(x)$.
The pseudo-code for the full algorithm is presented in \cref{alg:seqprox-sgd}.
\textcolor{red}{TODO: Sample two samples independently, one for constraint, one for objective.}
\begin{algorithm}[t]
...
\For{$k = 0$ to $K-1$}
    \State Sample $\xi_1^k, \xi_2^k \sim \xi$
    \State $g_k \gets \nabla F_{\xi_1^k}(x_{k}) + \gamma_k \nabla h_k(x_k; A(\xi_2^k), b(\xi_2^k))$
