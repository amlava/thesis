...
\begin{equation}
    ...
    \end{aligned}
\end{equation}
where $\xi$ is a random variable that captures uncertainty and
takes values in $\R^p$.
...
\begin{equation*}
    ...
    &\textup{s.\,t.} \quad A_j x - b_j \leq 0 \quad \textnormal{for all $j\in\{\, 1,\dots,N \,\}$}.
    \end{aligned}
\end{equation*}
...
\For{$k = 0$ to $K-1$}
    \State Sample $\xi_k$ from the distribution of $\xi$
    \State $g_k \gets \nabla F_{\xi_k}(x_{k}) + \gamma_k \nabla h_k(x_k; A(\xi_k), b(\xi_k))$
