...
\begin{assumption}
    ...
    \begin{enumerate}
        ...
        $\mu\in(0, \infty)$.
        \item The function $r$ is proper, convex, and locally Lipschitz continuous.
...
\begin{lemma}
    ...
    \begin{enumerate}
        ...
        \item The objectives $(f_k)_{k\in\N}$ of \eqref{eq:main-problem-penalized} are $\mu$-strongly convex, locally
            Lipschitz continuous, and subdifferentiable.
        \item The functions $(\psi_k)_{k\in\N}$ are $\mu$-strongly convex and
            $L_k$-smooth, where $L_k := L + \gamma_kL_{h_k}$.
            (\textcolor{red}{TODO: Remove smoothness when the assumption on $h_k$ is dropped.})
        \item Let $B\subset\R^n$ be a bounded subset. Then, for all $x\in B,
        \gamma\in[0,\infty), k\in\N$, the stochastic gradients
        \[
            g(x) := \nabla F_\xi(x) + \gamma \nabla h_k(A(\xi)x - b(\xi))
        \]
        satisfy $\sup_{x\in B} g(x) = \mathcal{O}(\gamma)$ almost surely.
        (\textcolor{red}{TODO: Maybe not needed to mention this particular subgradinet, since we actually only care about $\E||g_k||$.})
...
\begin{proof}
    ...
    \end{align*}
    Since $(x_k)_{k\in\N}$ is bounded almost surely (\cref{thm:almost-sure-convergence}),
    ...
    \begin{align*}
        \norm{x_{k+1} - x_k} &= \norm{x_{k+1} - y_k + y_k - x_k} \\
                            &= \norm{\prox(x_k - \eta_k g_k) - \prox(x_k)} + \norm{y_k - x_k} \\
                            &\leq \eta_k \norm{g_k} + \norm{y_k - x_k}.
    \end{align*}
    ...
    \]
    Finally, local Lipschitz continuity of $r$  (\cref{ass:basic-assumptions}) and yields (a.\,s.)
    \[
        L_r \norm{x_k - y_k} \geq r(x_k) - r(y_k) \geq \eta_k^{-1} \norm{x_k - y_k}^2
        \enspace\iff\enspace
        \norm{x_k - y_k} \leq \eta_k L_r,
    \]
    for some $L_r \in (0,\infty)$, where we again used that $(x_k)_{k\in\N}$ is bounded almost surely
...
\begin{proof}
    ...
    \]
    \textcolor{red}{TODO: $x_0$ need not be in the domain of $f$, so start the sum from $k_0 = 1$ to
    apply convexity.}
