...
as well as the expected value mapping,
are defined.
We denote the feasible set for \eqref{eq:main-problem-copy} as
\[
    \mathcal{X} := \{\, x\in\R^n \mid A(\xi)x - b(\xi) \leq 0 \enspace \textnormal{a.\,s.} \,\}.
\]
\begin{assumption}
    \label{ass:basic-assumptions}
    Throughout, we will make the following basic assumptions about problem \eqref{eq:main-problem}:
    \begin{enumerate}
        ...
        $\mu\in(0, \infty)$.
        \item The regularizer $r$ is convex and subdifferentiable with bounded subdifferential on compacta.
            More precisely, for any compact set $C\subset\R^n$, it holds that
            \[
                \sup_{x\in C}\sup_{\tilde{\nabla}r(x)\in\partial r(x)} \norm{\tilde{\nabla}r(x)} < \infty.
            \]
            \textcolor{red}{TODO: This is not actually needed. Closed, proper, convex (on the domain) are enough.}
        \item The matrix-valued map $A\colon\R^p\to\R^{m\times n}$, and the vector valued
            map $b\colon\R^p\to\R^m$, are both (Borel-)measurable.
        \item The random matrix $A(\xi)$ has bounded norm Frobenius norm, $\norm{A(\xi)}_F < \infty$, almost surely. \textcolor{red}{TODO: Remove this once you fixed the definition of $h_k$.}
        \item The sequence $(\gamma_k)_{k\in\N}$ is nondecreasing and unbounded. 
        \item There exists at least one feasible point $x\in\mathcal{X}$.
    \end{enumerate}
\end{assumption}
\Cref{alg:seqprox-sgd} work with penalty functions $(\pi_k)_{k\in\N}$.
These will always take the form $\pi_k(x) := \E(h_k(A(\xi)x - b(\xi)))$, where, for all $k\in\N$,
we let $(h_k)_{k\in\N}$ be any sequence of functions $\R^m\to\R$ with the following properties:
\begin{enumerate}
    \item For all $k\in\N$, $h_k$ is convex and $L_{h_k}$-smooth for some $L_{h_k}\in(0,\infty)$ (\textcolor{red}{TODO: Think convex differentiable is enough}).
    \item $h_k(x) \geq \norm{(x)_+}_1$ for all $x\in\R^m$ and $k\in\N$.
    \item There exists a sequence $(\alpha_k)_{k\in\N}\subset(0,\infty)$ such that
        $\sup_{k\in\N} \gamma_k\alpha_k < \infty$ and $h_k(x) \leq \alpha_k$ for all $x\in\R^m_{\leq 0}$.
        In particular, for all feasible points $\tilde{x}\in\mathcal{X}$, it holds that $\pi_k(\tilde{x}) \leq \alpha_k$ for all $k\in\N$.
    \item The gradients of $(h_k)_{k\in\N}$ are bounded uniformly:
        \[
            \sup_{k\in\N}\sup_{x\in\R^m}\nabla h_k(x) < \infty.
        \]
\end{enumerate}
Finally, for all $k\in\N$, we define the sequence of unconstrained optimization problems
\begin{equation}
...
\begin{lemma}
    ...
    \begin{enumerate}
        \item The objective $f$ of \eqref{eq:main-problem-copy} is $\mu$-strongly convex and subdifferentiable. Further,
            the subgradients of $f$ are bounded on compacta: For any compact set $C\in\R^n$, it holds that
            \[
                \sup_{x\in C}\sup_{\tilde{\nabla}f(x)\in\partial f(x)} \norm{\tilde{\nabla} f(x)} < \infty.
            \]
        \item The objectives $(f_k)_{k\in\N}$ of \eqref{eq:main-problem-penalized} are $\mu$-strongly convex
            and subdifferentiable. Further, for any compact set $C\in\R^n$, it holds that
            \[
                \sup_{x\in C} \sup_{\tilde{\nabla}f_k(x)\in\partial f_k(x)} \norm{\tilde{\nabla} f_k(x)} = \mathcal{O}(\gamma_k),
            \]
            for all $k\in\N$.
        \item The functions $(\psi_k)_{k\in\N}$ are $\mu$-strongly convex and
            $L_k$-smooth, where $L_k := L + \gamma_kL_{h_k}$. Further,
            for any compact set $C\subset\R^n$, it holds that
            \[
                \sup_{x\in C} \norm{\nabla \psi_k(x)} = \mathcal{O}(\gamma_k).
            \]
        \item The gradients of $(\pi_k)_{k\in\N}$ are uniformly bounded: There exists $G\in(0,\infty)$ such that
            \[
...
\end{equation}
we see that the two settings almost match, except for one important
difference: Cutler et al. \cite{cutler2023stochastic}
assume that the smoothness constant (as well as the strong convexity constant)
of $g_t$ is independent of $t\in\N$ (\textcolor{red}{TODO: Change this when the smoothness
assumption on $h_k$ is dropped}). In our case however,
the smoothness constant of $\psi_k$ depends on $\gamma_k h_k$, and
will generally grow over time. This will lead to a constraint
on the decay rate of our step size schedule $(\eta_k)_{k\in\N}$.
Namely, we will need $\eta_k\in (0, 1/L_k)$ with $L_k$
the smoothness constant of $\psi_k$, for all $k\in\N$.
Nevertheless, adapting the proof technique to our
case will not be very complicated, and will help us
establish a useful inequality. This inequality can then be used
...
conditions under which we can guarantee almost sure convergence of the sequence of iterates $(x_k)_{k\in\N}$.
The proof will also yield convergence in expectation of
$(x_k)_{k\in\N}$ to $x^\star$ along a subsequence. The main use of the almost sure convergence
result will be that we will have conditions on the stepsizes $(\eta_k)_{k\in\N}$ and the
penalty parameters $(\gamma_k)_{k\in\N}$ to ensure that $(x_k)_{k\in\N}$ is bounded with
...
\begin{lemma}[Robbins-Siegmund]
    ...
    Let $(\mathcal{F}_n)_{n\in\N}$ be an increasing sequence of $\sigma$-algebras and $v_n, a_n,
    b_n, c_n$ be nonnegative $\mathcal{F}_n$-measurable random variables. If, for all $n\in\N$,
    \begin{equation}
        \label{eq:lem:robbins-siegmund-almost-supermartingales}
        \E(v_{n+1} \mid \mathcal{F}_n) \leq v_n(1+a_n) + b_n - c_n,        
    \end{equation}
    and $\sum_{n=1}^\infty a_n < \infty, \sum_{n=1}^\infty b_n < \infty$ a.\,s., then with probability
    one, $(v_n)_{n\in\N}$ is convergent and it holds that $\sum_{n=1}^\infty c_n < \infty$.
\end{lemma}
\begin{proof}
...
sequence of solutions $(x_k^\star)_{k\in\N}$ to the sequence of unconstrained
problems \eqref{eq:main-problem-penalized}. We can then use this to "build a bridge"
between $(x_k)_{k\in\N}$ and $x^\star$, by considering their respective relationships to
the sequence $(x_k^\star)_{k\in\N}$.

The first step towards analyzing the convergence of $(x_k^\star)$
...
In \cite{hu1989approximate}, this result was then extended to the case of infinte systems
of linear inequalities, which is the setting that we are interested in.
However, before we can apply this inequality, we will first need to impose two assumptions. The first one concerns
the joint distribution of $A(\xi)$ and $b(\xi)$.
For $i\in\{\, 1,\dots,m \,\}$, $z\in\R^p$, we let $a_i(z)\in\R^{n}$, resp. $b_i(z)\in\R$, denote
the $i$-th row of $A(z)$, resp. $b(z)$.
\begin{assumption}
    \label{ass:compact-support-without-zero}
    There exists a compact set $C\in\R^{n+1}$ such that $0\notin C$ and the set
    \[
        \{\, (a_i(\xi), b_i(\xi)) \mid i\in\{\, 1,\dots,m \,\} \,\}
    \]
    is contained in $C$ almost surely. \textcolor{red}{TODO: This implies the bounded second moment assumption
    on $\E\norm{A(\xi)}_F^2$}.
\end{assumption}

The second assumption we make concerns the structure of the solution set $\mathcal{X}$
and is a very common assumption in optimization.
\begin{assumption}
...
\begin{lemma}[Subfeasibility bound via penalty functions]
    ...
    Assume \cref{ass:compact-support-without-zero,ass:slater-point} hold,
    let $B\subset\R^n$ be a bounded subset. Then
    there exists a constant $\tau\in(0,\infty)$ such that
    \[
       \tau \dist(x, \mathcal{X}) \leq \pi_k(x),
    \]
    for all $x\in B$ and $k\in\N$.
\end{lemma}
\begin{proof}
    % The projection map sends bounded sets to bounded sets. Indeed, if not,
    % then there exists a sequence $(y_k)_{k\in\N}\in B$ such that $\lim_{k\to\infty} \norm{\proj(y_k)} = \infty$.
    % But the projection map is contractive, hence, for any $x\in \mathcal{X}$,
    % \[
    %     \norm{y_k - x} \geq \norm{\proj(y_k) - x} \longrightarrow_{k\to\infty} \infty.
    % \]
    % This implies that $\lim_{k\to\infty}\norm{y_k} = \infty$, contradicting the assumption that $B$ is bounded.
    % Thus, the set $S := \{\, \proj(x) \mid x\in B \,\} \subset \mathcal{X}$ is bounded.
    % We begin by defining the following set:
    % \[
    %     \tilde{Y} := \{\, g_{(x, \lambda)}\colon\R^n\times\R\to \R \mid g_{(x, \lambda)}(a, b) := a^\top x - \lambda b
    %         \enspace \forall (x, \lambda), (a,b) \in \R^n \times \R \,\}.
    % \]
    % $\tilde{Y}$ is a vector space, since sums and scalar multiples of affine maps are affine.
    % It is also finite-dimensional, since for all $(x,\lambda)\in\R^n\times\R$, and standard basis vectors
    % $(e_1, \dots, e_n, 1)$, there exist constants $\lambda_1,\dots,\lambda_n\in\R$,
    % such that
    % \[
    %     g_{(x, \lambda)}(a, b) = a^\top x - \lambda b = \sum_{i=1}^n \lambda_i a^\top e_i - \lambda b = \sum_{i=1}^n \lambda_i g_{(e_i, 0)}(a, b) - \lambda g_{(0, 1)}(a, b).
    % \]
    % Thus, the family $\{\, g_{(e_i, 0)}, g_{(0, 1)} \mid i\in\{1,\dots,n+1\} \,\}$ forms a basis for $\tilde{Y}$.
    % It also follows that the map
    % \[
    %     \tilde{T}\colon\R^{n}\times \R \to \tilde{Y}, \enspace \tilde{T}(x, \lambda) := g_{(x, \lambda)}
    % \]
    % defines a linear isomorphism.
    % We further define the subset
    % \[
    %     \tilde{K} := \{\, g_{(x,\lambda)} \in \tilde{Y} \mid g_{(x,\lambda)}(a, b) \leq 0
    %     \enspace \forall (a, b) \in C \,\}.
    % \]
    % $\tilde{K}$ is a closed (wrt. some arbitrary norm on $\tilde{Y}$) convex cone: The latter two properties are clear from
    % the definition. For closedness, let $(x_k, \lambda_k)_{k\in\N}$ be a sequence in $\R^n\times\R$ such
    % that the corresponding sequence of functions $(g_{(x_k, \lambda_k)})_{k\in\N}$ is a sequence in $\tilde{K}$ that
    % converges to $g_{(x, \lambda)}\in\tilde{Y}$ for some $(x, \lambda)\in\R^n\times\R$.
    % Since we have already established that $\tilde{Y}$ is isomorphic to $\R^{n+1}$ via $\tilde{T}$,
    % the convergence of $(g_{(x_k, \lambda_k)})_{k\in\N}$ to $g_{(x, \lambda)}$ is equivalent to the convergence
    % of $(x_k, \lambda_k)_{k\in\N}$ to $(x, \lambda)$ in $\R^n\times\R$.
    % Therefore, we can conclude that for any $(a, b)\in\R^{n}\times\R$, we have
    % \[
    %     \lim_{k\to\infty} a^\top x_k - \lambda_k b = a^\top x - \lambda b,
    % \]
    % and $a^\top x - \lambda b$ must also be nonnegative, proving that $g_{(x,\lambda)}\in \tilde{K}$.
    % Further, we define the maps
    % \[
    %     \tilde{A} \colon \R^n\to \tilde{Y}, \enspace \tilde{A}x := g_{(x, 0)},
    % \]
    % and
    % \[
    %     \tilde{b} := g_{(0, 1)}.
    % \]
    % For $x_0\in B$, we consider the optimization problem
    % \begin{equation}
    %     \label{eq:proof:lem:lower-bound-on-penalties-by-distance-to-feasibility}
    %     \min_{x\in\mathcal{X}} \norm{x - x_0}
    %     =
    %     \min_{x\in\R^n, \, \tilde{A}x - \tilde{b} \in \tilde{K}} \norm{x - x_0},
    % \end{equation}
    % where the equality follows from the fact that $x\in\mathcal{X} \iff g_{(x, 1)}  = \tilde{A}x - \tilde{b} \in \tilde{K}$.
    % Define the map
    % We will now show that $\tilde{T}$ is a linear isomorphism: For $x,y\in\R^n$, and $\lambda,\mu\in\R$,
    % it holds that
    % \begin{align*}
    %     \tilde{T}(x + y, \lambda + \mu)(a, b)
    %     &= g_{(x + y, \lambda + \mu)}(a, b) \\
    %     &= a^\top (x + y) - (\lambda + \mu)b \\
    %     &= a^\top x - \lambda b + a^\top y - \mu b \\
    %     &= g_{(x, \lambda)}(a, b) + g_{(y, \mu)}(a, b) \\
    %     &= \tilde{T}(x, \lambda)(a, b) + \tilde{T}(y, \mu)(a, b),
    % \end{align*}
    % for all $(a,b)\in\R^{n}\times\R$. Further, for any $x\in\R^n$, $\lambda,\mu\in\R$,
    % \begin{align*}
    %     \tilde{T}(\mu x, \mu \lambda)(a, b) &= g_{(\mu x, \mu \lambda)}(a, b) \\
    %                 &= a^\top (\mu x) - (\mu\lambda)b \\
    %                 &= \mu( a^\top x - \lambda b ) \\
    %                 &= \mu g_{(x, \lambda)} \\
    %                 &= \mu \tilde{T}(x, \lambda)(a, b),
    % \end{align*}
    % for all  $(a,b)\in\R^{n}\times\R$. Thus, $\tilde{T}$ is linear. Now assume that
    % \[
    %     0 = \tilde{T}(x, \lambda)(a, b) = g_{(x, \lambda)}(a, b) = a^\top x - \lambda b \enspace \forall (a,b)\in\R^n\times\R
    % \]
    % for some $(x, \lambda) \in \R^n\times\R$. Then, in particular,
    % \[
    %     0 = g_{(x, \lambda)}(x, -\lambda) = \norm{x}^2 + \lambda^2,
    % \]
    % which implies that $x = 0$ and $\lambda = 0$. Thus, $\tilde{T}$ is injective, and since
    % $\dim(\tilde{Y}) = n + 1$, it follows that $\tilde{T}$ must also be surjective, and hence $\tilde{T}$
    % is an isomorphism. We can thus identify the set $\tilde{Y}$ with $\R^{n+1}$ via
    % \[
    %     Y := \tilde{T}^{-1}(\tilde{Y}) = \R^n \times \R \cong \R^{n+1},
    % \]
    % and we similarly identify $\tilde{K}$ with a subset of $\R^{n+1}$ via
    % \[
    %     K := \tilde{T}^{-1}(\tilde{K})
    %     = \{\, (x, \lambda) \in \R^{n+1} \mid g_{x, \lambda}(a, b) \leq 0 \enspace \forall (a, b)\in\R^n\times\R \,\}.
    % \]
    % Note that $\tilde{A}x - \tilde{b} \in
    % \tilde{K} \iff (\tilde{T}^{-1}\tilde{A})x - (\tilde{T}^{-1}\tilde{b}) \in \tilde{T}^{-1}(\tilde{K})$,
    % which motivates the definitions (with slight abuse of notation)
    % \[
    %     A := \tilde{T}^{-1}\tilde{A}, \enspace \enspace b := \tilde{T}^{-1} \tilde{b}, \enspace \text{and} \enspace
    %     K := \tilde{T}^{-1}(\tilde{K}).
    % \]
    % With this, the minimization problem \eqref{eq:proof:lem:lower-bound-on-penalties-by-distance-to-feasibility}
    % can be rewritten as
    % \[
    %     \min_{x\in\mathcal{X}} \norm{x - x_0} = \min_{x\in\R^n, \, Ax - b \in K} \norm{x - x_0}.
    % \]
    % The advantage of this formulation as opposed to the one involving $\tilde{K}$ is that
    % now the set $K$ is a genuine subset of $\R^{n+1}$, which will prove useful in the following.
    % Not also that the set $K$ is a closed convex cone, since linear isomorphisms preserve all these properties and we
    % have already shown that $\tilde{K}$ is a closed convex cone.
\end{proof}
\begin{theorem}[Convergence of $x_k^\star$]
    \label{thm:distance-to-feasibility-bound}
    Assume \cref{ass:slater-point,ass:compact-support-without-zero} hold.
...
\end{theorem}
\begin{proof}
    Let $k\in\N$. By optimality of $x_k^\star$ for $f_k$, there exists
    ...
    It follows that $(\proj(x_k^\star))_{k\in\N}$ is bounded as well (continuous functions map compact sets to compact sets),
    and therefore there exists a constant $M\in(0,\infty)$ such that $M = \sup_{k\in\N} \norm{\tilde{\nabla} f(\proj(x_k^\star))}$.
    Hence, by
    convexity of $f$ and Cauchy-Schwarz,
    \[
        f(\proj(x_k^\star)) - f(x_k^\star) \leq \langle \tilde{\nabla} f(\proj(x_k^\star)), \proj(x_k^\star) - x_k^\star \rangle
                                    \leq M \dist(x_k^\star).
    \]
    We now obain
    \[
...
Having established the convergence of the sequence $(x_k^\star)_{k\in\N}$
to $x^\star$, we will now shift our attention to the iterates $(x_k)_{k\in\N}$
...
\begin{proof}
    ...
    $\nabla \psi_k = \nabla \psi + \gamma_k \nabla\pi_k$. By $L$-smoothness of $\psi$ (\cref{ass:basic-assumptions}),
    we have
    \begin{align*}
        f_k(x_{k+1})
        &= \psi_k(x_{k+1}) + r(x_{k+1}) \\
    ...
    Note that the gradients $(\nabla \pi_k)_{k\in\N}$ are bounded uniformly (\cref{lem:basic-properties}),
    ...
    \]
    where again used the bound $\sup_{k\in\N}\sup_{x\in\R^n}\nabla \pi_k(x) \leq G$, and therefore we obtain
    \begin{align*}
        f_k(x_{k+1})
    ...
    \end{align*}
    for all $x\in\R^n$. Since $\E_k\norm{z_k}^2 = \E_k\norm{g_k - \nabla \psi_k(x_k)}^2$ and
    $\E_k(g_k) = \nabla \psi_k(x_k)$ per our definition of stochastic gradients, it holds that $\E_k\norm{z_k}^2 = \Var_k(g_k)$.
...
\begin{lemma}
    ...
    Assume that \cref{ass:slater-point,ass:compact-support-without-zero} hold, and
    let $\eta_k\in(0,\rho L^{-1}]$ for all $k\in\N$ and some $\rho\in(0,1)$. Then the iterates $(x_k)_{k\in\N}$ generated
    by \cref{alg:seqprox-sgd} with step size schedule $(\eta_k)_{k\in\N}$ satisfy
    \begin{align*}
        \E_k\norm{x^\star - x_{k+1}}^2
        &\leq
        (1-\mu\eta_k)\norm{x^\star - x_k}^2
...
\end{lemma}
\begin{proof}
    Let $k\in\N$. From \cref{lem:one-step-improvement}, we have
    \begin{align*}
        2\eta_k\E_k(f_k(x_{k+1}) - f_k(x^\star))
        &\leq
            (1-\mu\eta_k)\norm{x^\star - x_k}^2
    ...
    In particular, $(\proj(x_k^\star))_{k\in\N}$ is bounded, so by \cref{lem:basic-properties},
    it follows that $\sup_{k\in\N} \norm{\tilde{\nabla} f_k(\proj(x_k^\star))} \leq c \gamma_k$
    for some $c\in(0,\infty)$. Hence,
    \[
        f_k(x_k^\star) - f_k(\proj(x_k^\star)) \geq -c \gamma_k\, \dist(x_k^\star,\mathcal{X}).
    \]
    By \cref{thm:distance-to-feasibility-bound}, it holds that there exist
    $K\in\N$ and $c^\prime\in(0,\infty)$ such that, for all $k\geq K$, $\dist(x_k^\star,\mathcal{X}) \leq c^\prime \, \alpha_k$.
    Therefore,
    \[
        f_k(x_k^\star) - f_k(\proj(x_k^\star)) \geq -cc^\prime \cdot \gamma_k \alpha_k
    \]
    and
    \begin{align*}
        f_k(x_{k+1}) - f_k(x^\star)
        ...
        -cc^\prime\cdot \gamma_k\alpha_k - \gamma_k\alpha_k \\
        &=
        f_k(x_{k+1}) - f_k(x_k^\star)
        -(1 + cc^\prime) \, \gamma_k\alpha_k,
    \end{align*}
    for all $k\geq K$.
    ...
    \begin{align*}
        ...
        - (1 + cc^\prime)\gamma_k\alpha_k\big)
        &\leq
        (1-\mu\eta_k)\norm{x^\star - x_k}^2
        - \E_k\norm{x^\star - x_{k+1}}^2 \\
    ...
    \begin{align*}
        ...
        (1-\mu\eta_k)\norm{x^\star - x_k}^2
        + \frac{2\eta_k^2}{1 - \rho}\Var_k(g_k) + \frac{8 G^2\eta_k^2\gamma_k^2}{1 - \rho} \\
        &\phantom{===} + 2(1 + cc^\prime)\,\eta_k\gamma_k\alpha_k
...
for $f_k$. Assuming that $\sum_{k=1}^\infty \eta_k\gamma_k\alpha_k < \infty$ and
$\sum_{k=1}^\infty \eta_k^2 \gamma_k^2 < \infty$, we are almost ready
...
\begin{lemma}[Bound on gradient noise]
    ...
    In the situation of \cref{alg:seqprox-sgd}, there
...
\end{lemma}
\begin{proof}
    Let $k\in\N$ and define $\tilde{g}_k := \nabla F_\xi(x_k) + \gamma_k A(\xi)^\top \nabla h_k(A(\xi)x_k - b(\xi))$.
    We have
    \begin{align*}
        \Var_k(g_k)
        &=
    ...
    \]
    Now, we can use the fact that $\norm{A v}\leq \norm{A}_F \norm{v}$ for any matrix
    $A$ and vector $v$, to find
    \[
        \E_k\norm{A(\xi)^\top \nabla h_k(A(\xi)x_k - b(\xi))}^2
        \leq
        \E_k\Big( \norm{A(\xi)}_F^2\norm{\nabla h_k(A(\xi)x_k - b(\xi))}^2 \Big).
    \]
    Since, per assumption, it holds that a) the family $(h_k)_{k\in\N}$ has uniformly bounded gradients,
    and b) $\E\norm{A(\xi)}_F^2 < \infty$,  there exists a constant
    $\tilde{M}^2\in(0,\infty)$ such that
    \[
        \E_k\norm{A(\xi)^\top \nabla h_k(A(\xi)x_k - b(\xi))}^2 \leq \tilde{M}^2.
    \]
...
\begin{theorem}[Almost sure convergence]
    ...
    Assume \cref{ass:slater-point,ass:compact-support-without-zero} hold.
    Let $(x_k)_{k\in\N}$ be a sequence generated by \cref{alg:seqprox-sgd} with
    parameters $(\eta_k)_{k\in\N}$, $(\gamma_k)_{k\in\N}$, $(h_k)_{k\in\N}$ that satisfiy
    \begin{enumerate}
        \item $\sum_{k=1}^\infty \eta_k = \infty$ and there exists $\rho\in(0,1)$ such that $\eta_k \leq \rho L^{-1}$ for all $k\in\N$.
        \item $\sum_{k=1}^\infty \eta_k\gamma_k\alpha_k < \infty$.
        \item $\sum_{k=1}^\infty \eta_k^2 \gamma_k^2 < \infty$.
    \end{enumerate}
...
\begin{proof}
    ...
    \[
        \Var_k(g_k) \leq \frac{2L^2}{\beta_k} \norm{x_k - x^\star}^2 + \frac{1 + \gamma_k^2}{\beta_k} \sigma^2,
    \]
    for a constant $M^2\in(0,\infty)$ and all $k\in\N$.
    Thus, we have
    \begin{align*}
        \E_k\norm{x^\star - x_{k+1}}^2
        &\leq
    ...
    \end{align*}
    for all $k\in\N$.
    Define the nonegative sequences $(a_k)_{k\in\N}, (b_k)_{k\in\N}, (c_k)_{k\in\N}$ by
    \begin{align*}
    ...
    \[
        ...
        + b_k - c_k.
    \]
    Our assumptions imply that $\sum_{k=1}^{\infty} a_k < \infty$ and $\sum_{k=1}^\infty b_k < \infty$,
    so we can apply
    \cref{lem:robbins-siegmund}, which implies that with probability one the sequence
    $\norm{x^\star - x_{k}}^2$ converges and
    $\sum_{k=1}^\infty \eta_k\E_k(f_{k}(x_{k+1}) - f_{k}(x_k^\star)) < \infty$.
    By the bounded convergence theorem (\textcolor{red}{TODO: add ref}), it further holds that
    \begin{align*}
        \infty > \E\Big( \sum_{k=1}^\infty \eta_k\E_k(f_{k}(x_{k+1}) - f_{k}(x_k^\star)) \Big)
        &=
        \sum_{k=1}^\infty \eta_k\E(\E_k(f_{k}(x_{k+1}) - f_{k}(x_k^\star))) \\
        &=
        \sum_{k=1}^\infty \eta_k\E(f_{k}(x_{k+1}) - f_{k}(x_k^\star)).
    \end{align*}
    Since $\sum_{k=1}^\infty \eta_k = \infty$, it must therefore hold that
    \[
...
\begin{assumption}
    ...
    The sequence $(h_k)_{k\in\N}$
    and the
    parameters $(\eta_k)_{k\in\N}$, $(\gamma_k)_{k\in\N}$ from \cref{alg:seqprox-sgd} satisfiy
    \begin{enumerate}
        \item $\sum_{k=1}^\infty \eta_k = \infty$ and there exist constants $\rho\in(0,1)$ and $K\in\N$ such that $\eta_k \leq \rho L^{-1}$ for all $k\in\N, k\geq K$.
        \item $\sum_{k=1}^\infty \eta_k\gamma_k\alpha_k < \infty$.
        \item $\sum_{k=1}^\infty \eta_k^2 \gamma_k^2 < \infty$.
    \end{enumerate}
\end{assumption}
...
\begin{lemma}
    ...
    Let \cref{ass:compact-support-without-zero,ass:slater-point,ass:step-sizes-and-penalties} hold.
    Then there exists a deterministic sequence $(M_k^2)_{k\in\N} \subset \R^n$ with
    ...
    \]
    for all $k\in\N$ and $x\in\R^n$.
\end{lemma}
\begin{proof}
    Let $k\in\N, k\geq K$. Taking expectations on both sides of the inequality provided by
    ...
    for all $k\in\N$ (almost surely).  We can assume without loss of generality that $\sigma^2 \leq \tilde{\sigma}^2$,
    ...
    \]
    for all $x\in\R^n$. By convexity of $f_k$ and Cauchy-Schwarz, we have
    \[
        f_k(x_{k+1})
        \geq
    ...
    \end{align*}
    By definition of $x_{k+1}$ as the solution to $\min_{x\in\R^n} \{ r(x) + (2\eta_k)^{-1} \norm{x - (x_k - \eta_k g_k)}^2 \}$,
    with probability one there exists a subgradient $\tilde{\nabla} r(x_{k+1}) \in \partial r(x_{k+1})$, such that
    \[
        \tilde{\nabla}r(x_{k+1}) + \frac{1}{\eta_k} (x_{k+1} - x_k + \eta_k g_k) = 0
        \iff
        x_{k+1} - x_k = -\eta_k (\tilde{\nabla}r(x_{k+1}) + g_k)
    \]
    Therefore,
    \[
        \norm{x_{k+1} - x_k} \leq \eta_k \big( \norm{\tilde{\nabla}r(x_{k+1})} + \norm{g_k} \big) \enspace\text{(a.\,s.)}.
    \]
    Again using that $(x_{k})_{k\in\N}$ is bounded almost surely,
    \cref{lem:basic-properties} implies
    \[
        \norm{g_k} = \norm{\nabla \psi_k(x_k)} = \mathcal{O}(\gamma_k),
        \enspace
        \sup_{k\in\N} \norm{\tilde{\nabla}r(x_{k})} < \infty,
        \enspace \text{and} \enspace
        \sup_{k\in\N} \norm{\tilde{\nabla} f_k(x_k)} = \mathcal{O}(\gamma_k),
    \]
    and thus $\norm{\tilde{\nabla} f_k(x_k)} \norm{x_{k+1} - x_k} = \eta_k \mathcal{O}(\gamma_k^2)$ almost surely.
    It follows that
    \begin{align*}
        2\eta_k\E(f_k(x_{k}) - f_k(x))
        &\leq
    ...
    \end{align*}
    Therefore, there exists a sequence $(M_k^2)_{k\in\N} \subset \R_{\geq 0}$ such that
...
\begin{lemma}
    ...
    Let \cref{ass:compact-support-without-zero,ass:slater-point}
    ...
    with parameters $(\eta_k)_{k\in\N_0}$, $(\gamma_k)_{k\in\N_0}$, $(\beta_k)_{k\in\N_0}$
    satisfy
    \[
        \E(f(\bar{x}_{K}) - f(x^\star))
        \leq
        \frac{S_{0, k_0}}{S_K} \E(f(\bar{x}_{0, k_0}) - f(x^\star)) +
        \frac{e_{k_0}}{2S_K} + \frac{\sum_{k=k_0}^{K-1} M_k^2}{2S_K} + \frac{\sum_{k=k_0}^{K-1}\eta_k^{-1}\gamma_k\alpha_k}{S_K},
    \]
    for all $K\in\N$, where $S_K := \sum_{k=0}^{K-1} \eta_k^{-1}$,
    ...
    \[
        ...
        \leq
        \frac{S_{0, k_1}}{S_K} \E(\dist(\bar{x}_{0, k_1}))
...
\begin{proof}
    ...
    \[
        ...
        = \eta_{k-1}^{-2}.
    \]
    Setting $e_j := \eta_{j-1}^{-2} \E\norm{x^\star - x_j}^2$ for all $j\in\N_0$, we have
    \[
        2\eta_k^{-1} \E(f(x_{k}) - f(x^\star))
        \leq
        e_k - e_{k+1} + M_k^2 + 2\eta_k^{-1}\gamma_k\alpha_k.  
    \]
    Summing both sides over $k = k_0,\dots,K-1$ for $K\in\N$ yields
    \begin{align*}
        2\sum_{k=k_0}^{K-1} \eta_k^{-1} \E(f(x_{k}) - f(x^\star))
        &\leq
        e_{k_0} - e_{K} + \sum_{k=k_0}^{K-1} M_k^2 + 2\sum_{k=k_0}^{K-1}\eta_k^{-1}\gamma_k\alpha_k \\
        &\leq
        e_{k_0} + \sum_{k=k_0}^{K-1} M_k^2 + 2\sum_{k=k_0}^{K-1}\eta_k^{-1}\gamma_k\alpha_k,
    \end{align*}
    where we used $e_K \geq 0$ in the second step.
    We define $S_{t, k} := \sum_{i=t}^{k-1} \eta_i^{-1}$, $S_k := S_{0, k}$,
    and $\bar{x}_{t, k} := S_{t, k}^{-1} \sum_{i=t}^{k-1} \eta_i^{-1} x_i$ for $t,k\in\N$.
    Using convexity of $f$, we get
    \begin{align*}
        \E(f(\bar{x}_{k_0,K}) - f(x^\star))
        &\leq
        S_K^{-1}\sum_{k=k_0}^{K-1} \eta_k^{-1} \E(f(x_{k}) - f(x^\star)) \\
        &\leq
        \frac{e_{k_0}}{2S_{k_0, K}} + \frac{\sum_{k=k_0}^{K-1} M_k^2}{2S_{k_0, K}} + \frac{\sum_{k=k_0}^{K-1}\eta_k^{-1}\gamma_k\alpha_k}{S_{k_0, K}},
    \end{align*}
    ...
    % thus
    \[
        \bar{x}_K = \frac{S_{0, k_0}}{S_K} \bar{x}_{0, k_0} + \frac{S_{k_0, K}}{S_K} \bar{x}_{k_0, K}
    \]
    and
    \[
        \frac{S_{0, k_0}}{S_K} + \frac{S_{k_0, K}}{S_K} = 1,
    \]
    ...
    \[
        ...
        \frac{S_{0, k_0}}{S_K} \E(f(\bar{x}_{0, k_0}) - f(x^\star))
        +
        \frac{e_{k_0}}{2S_{K}} + \frac{\sum_{k=k_0}^{K-1} M_k^2}{2S_{K}} + \frac{\sum_{k=k_0}^{K-1}\eta_k^{-1}\gamma_k\alpha_k}{S_{K}}.
    \]
    ...
    \end{align*}
    Using convexity of $f$, Cauchy-Schwarz, almost sure boundedness of $(x_k)_{k\in\N}$ (\cref{thm:almost-sure-convergence}), and the fact that
    subgradients of $f$ are bounded on compacta (\cref{lem:basic-properties}), there exists a constant $M\in(0,\infty)$,
    such that
    \[
        f(\proj(x_k)) - f(x_{k}) \leq \langle \tilde{\nabla} f(\proj(x_k)), \proj(x_k) - x_k \rangle
                                \leq M \dist(x_k, \mathcal{X}).
    \]
    ...
    \]
    for all $k\geq k_1$. Summing over $k=k_1,\dots, K - 1$ for $K\in\N$, we have
    \begin{align*}
        \tau \sum_{k=k_1}^{K - 1} \eta_k^{-1} \E(\dist(x_k, \mathcal{X}))
        &\leq
        \sum_{k=k_1}^{K - 1} (d_k - d_{k+1}) + \sum_{k=k_1}^{K - 1} \gamma_k^{-1}M_k^2 + 2 \sum_{k=k_1}^{K - 1}\eta_k^{-1}\alpha_k \\
        &=
        d_0 - d_K + \sum_{k=k_1}^{K - 1} \gamma_k^{-1}M_k^2 + 2 \sum_{k=k_1}^{K - 1}\eta_k^{-1}\alpha_k \\
        &\leq
        d_0 + \sum_{k=k_1}^{K - 1} \gamma_k^{-1}M_k^2 + 2 \sum_{k=k_1}^{K - 1}\eta_k^{-1}\alpha_k,
    \end{align*}
    ...
    \[
        ...
        \frac{\tau^{-1}d_{k_1}}{S_{k_1,K}}
            + \frac{\tau^{-1}\sum_{k=k_1}^{K - 1} \gamma_k^{-1}M_k^2}{S_{k_1,K}}
            + \frac{2\tau^{-1}\sum_{k=k_1}^{K - 1}\eta_k^{-1}\alpha_k}{S_{k_1,K}}.
    \]
    ...
    % thus
    \[
        \bar{x}_K = \frac{S_{0, k_1}}{S_K} \bar{x}_{0, k_1} + \frac{S_{k_1, K}}{S_K} \bar{x}_{k_1, K}
    \]
    and
    \[
        \frac{S_{0, k_1}}{S_K} + \frac{S_{k_1, K}}{S_K} = 1.
    \]
...
\begin{proof}
    ...
    Furthermore, \cref{lem:basic-properties} implies that we can bound the subgradients
    in $\partial f(x^\star)$ by a constant $M\in(0,\infty)$. Combining these facts
    with the above inequality, and applying Cauchy-Schwarz,
    we obtain
    \[
        f(\bar{x}_k) - f(x^\star) \geq - M \dist(\bar{x}_k),
    \]
    ...
    Since $\eta_k^{-1} = \mathcal{O}(k)$, it follows that $S_K = \mathcal{O}(K^2)$. Also, since
    $\alpha_k = \mathcal{O}(k)$, we have $\eta_k^{-1} \alpha_k \leq c$ for some $c\in (0,\infty)$.
    Further, $\gamma_k^{-1}M_k^2 = \mathcal{O}(\gamma_k)$. Therefore
    \begin{align*}
        \E(\dist(\bar{x}_K, \mathcal{X}))
        &\leq
        \mathcal{O}(K^{-2})
    ...
    Therefore,
    \[
        f(\bar{x}_k) - f(x^\star) \geq - M \cdot \mathcal{O}( \log^e(K) / K ).
    \]
...
\begin{lemma}
    ...
    \[
        \norm{z_k}^2 \leq 8\hat{L}^2 + 8\gamma_k^2N^2G^2,
    \]
    with probability one, where $\hat{L}, G, N\in(0,\infty)$.
\end{lemma}
\begin{proof}
    ...
    \begin{align*}
        \norm{z_k}^2
        &= \norm{g_k - \nabla \psi_k(x_k)}^2 \\
        &= \norm{\nabla F_{\xi_k}(x_k) + \gamma_k A(\xi_k)^\top \nabla h_k(A(\xi_k) x_k - b(\xi_k))
            + \nabla \E(F_{\xi_k}(x_k)) + \gamma_k \nabla \pi_k(x_k)}^2 \\
        &\leq 4 \big( \norm{\nabla F_{\xi_k}(x_k)}^2 + \gamma_k^2 \norm{A(\xi_k)}_F^2\norm{\nabla h_k(A(\xi_k) x_k - b(\xi_k))}^2 \\
            &\phantom{===}+ \norm{\nabla \E(F_{\xi_k}(x_k))}^2 + \gamma_k^2 \norm{\nabla \pi_k(x_k)}^2 \big)
    \end{align*}
    By almost sure $L$-smoothness of $x\mapsto F_\xi(x)$ and
    $x\mapsto \E(F_\xi(x))$ (\cref{ass:basic-assumptions}),
    as well as the fact that $(x_k)_{k\in\N}$ is almost surely bounded (\cref{thm:almost-sure-convergence}),
    there exists a constant $\hat{L}\in(0,\infty)$ such that
    \[
        \norm{\nabla F_{\xi_k}(x_k)} \leq \hat{L}
        \enspace
        \text{and}
        \enspace
        \norm{\nabla \E(F_{\xi_k}(x_k))} \leq \hat{L}.
    \]
    Therefore,
    \[
        \norm{\nabla F_{\xi_k}(x_k) + \gamma_k A(\xi_k)^\top \nabla h_k(A(\xi_k) x_k - b(\xi_k))}
        ...
        \hat{L} + \gamma_k N G.
    \]
    Further $\norm{A(\xi)}_F \leq N$ a.\,s. (\cref{ass:basic-assumptions}), and
    \[
        \sup_{k\in\N}\sup_{x\in\R^m} \norm{\nabla h_k(x)} \leq G
        \enspace
        \text{and}
        ...
        \sup_{k\in\N}\sup_{x\in\R^n} \norm{\nabla \pi_k(x)} \leq \E\norm{A(\xi)}_F G
    \]
    for a constant $G\in(0,\infty)$ (\cref{lem:basic-properties}).
    We thus obtain
    \[
        \norm{z_k}^2
        \leq
        4(2 \hat{L}^2 + 2\gamma_k^2N^2G^2) = 8\hat{L}^2 + 8\gamma_k^2N^2G^2,
    \]
    as desired.
\end{proof}
