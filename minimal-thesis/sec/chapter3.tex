...
These will always take the form $\pi_k(x) := \E(h_k(x; A(\xi), b(\xi)))$, where, for all $k\in\N$,
we let $(h_k)_{k\in\N}$ be any sequence of functions $\R^n\to\R$ with the following properties:
\begin{enumerate}
    ...
    \item $h_k(x) \geq \norm{(x)_+}_1$ for all $x\in\R^n$ and $k\in\N$.
...
\begin{proof}
    ...
    $\tilde{g}_k := \nabla F_{\xi_1}(x_k) + \gamma_k \nabla h_k(x_k; A(\xi_2), b(\xi_2))$.
    We have
    \begin{align*}
        \Var_k(g_k)
        &=
    ...
    \]
    Since, per assumption, it holds that the family $(h_k)_{k\in\N}$ has uniformly bounded gradients,
...
\begin{theorem}[Convergence rates in expectation]
    ...
    \[
        \E(\dist(\bar{x}_{K}, \mathcal{X})) = \mathcal{O} ( \log^{e}(K) / K ).
    \]
\end{theorem}
\begin{proof}
...
\begin{lemma}
    ...
    \]
    with probability one, where $L, G, N\in(0,\infty)$.
\end{lemma}
\begin{proof}
    ...
    we can locally bound the gradients
    \[
        \norm{\nabla F_{\xi_k}(x_k)} \leq L
        \enspace
        \text{and}
        \enspace
        \norm{\nabla \E(F_{\xi_k}(x_k))} \leq L.
    \]
    Therefore,
    \[
    ...
    \[
        \norm{z_k}^2
        \leq
        4(2 L^2 + 2\gamma_k^2G^2) = 8L^2 + 8\gamma_k^2G^2,
    \]
    as desired.
\end{proof}
